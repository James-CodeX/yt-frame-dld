...
def get_video_stream_url(youtube_url: str) -> str:
    """Get direct video stream URL using yt-dlp for 720p."""
    ydl_opts = {
        'format': 'best[height=720][ext=mp4]',  # Select 720p MP4 format
        'quiet': True,
    }
    try:
...
@app.post("/extract_frame_720p")
async def extract_frame_endpoint(request: VideoRequest):
    """API endpoint to extract a 720p frame from a YouTube video at a given timestamp."""
    ...
    object_name = f"frame_720p_{timestamp_str}.jpg"
